...
public class ArticleService {
	...
	public List<Article> getArticlesByUser(User user){
		return articleRepo.findArticleByOrderByUserDesc(user);
	}
	...
	public List<ArticleInfo> getArticleInfoByUserAndType(long userId, String type, String own ) {
		...
		if(own.equals("mine")) {
			if(type.equals("A")) {
				own_articles = articleRepo.findArticleByOrderByUserDesc(presentUser);
			...
			for(long fd : followee_ids) {
				...
				if(type.equals("A")) {
					own_articles = articleRepo.findArticleByOrderByUserDesc(followee);
					for(Article a : own_articles) {
						User user = a.getUser();
						// 유저가 해당 게시글에 하트를 눌렀는지 여부 확인
						boolean isLike = false;
